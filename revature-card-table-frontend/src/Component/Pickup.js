...
export default function Pickup({ token }) {
  ...
  useEffect(() => {
    axios
      .get("https://deckofcardsapi.com/api/deck/new/shuffle/", {
        params: {
          cards:
            "AS,AD,2D,2C,3S,3C,4H,4S,5D,5C,6S,6H,7H,7C,8D,8S,9H,9D,0C,0S,JH,JC,QH,QS,KD,KC,KH,KS,AS,AD,2D,2C,3S,3C,4H,4S,5D,5C,6S,6H",
        },
      })
      .then((response) =>
        axios.get(
          `https://deckofcardsapi.com/api/deck/${response.data.deck_id}/draw`,
          {
            params: {
  ...
  function play() {
    ...
    if (userMoves === 39) {
      ...
      axios
        .get(
          `http://localhost:8080/revature-card-table/users/${token.username}`
        )
        .then((response) => {
          ...
          {gameOver ? (
            <>
              <Alert
                id="alert"
                variant="primary"
                style={{ backgroundColor: "#1d1d35", color: "white" }}
              >
                Winner! Time to complete: {timeCompleted}!
              </Alert>
            </>
          ) : (
            <>
