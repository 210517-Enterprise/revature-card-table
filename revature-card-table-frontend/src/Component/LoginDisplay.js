...
export default function LoginDisplay({ setToken }) {    
  ...
  const onSubmit = (user) => {
    if(user.username && user.password){
      axios.get(`http://localhost:8080/revature-card-table/users/login?username=${user.username}&password=${user.password}`)
        .then((response) => {
    ...
    if(!user.username){
      let error_p = document.getElementById("error");
      error_p.innerText = "Authentication Error -  Complete the form to login";
      error_p.style.color = "#ff968f";
    ...
    if(!user.username){
      ...
      let usr_input = document.getElementById("password");
      usr_input.value = "";
      usr_input.style.backgroundColor = "#ff968f";
    }
  }
  ...
  return (
    <>
      <div id="page-layout" class="row h-100" padding-top="10em">
        <motion.div class="col-lg-6 mx-auto" variants={fadeLeft} initial="hidden" animate="visible"
          ...
          <Form>
            ...
            <Form.Group>
              ...
              <br />
              <h5>Username</h5>
              <Form.Control id="username" type="text" placeholder="Enter username" onClick={enterUsername}
                {...register("username", { required: false })} />
            </Form.Group>

            <Form.Group>
              <h5>Password</h5>
              <Form.Control id="password" type="password" placeholder="Enter password" onClick={enterPassword}
                {...register("password", { required: false })} />
            </Form.Group>

            <p id="error"></p>
            <motion.button className="btn btn-primary btn-block" type="submit" onClick={handleSubmit(onSubmit)}
