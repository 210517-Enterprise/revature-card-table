...
export default function War({ token }) {
  ...
  function assessValue() {
    ...
          <h2 className = "return">Player Card Remaining {playerDeck.length} | Player Score: {playerScore}</h2>
          <img src={playerDeck[0].image }  style={{width:"226px"}}></img>
          <div>
              <h2 className = "return">Previous Round Winner: {currentWinner}</h2>
          </div>
          <div className="AICard">
            <h2 className = "return">AI Card r: {AiDeck.length} | AI Score: {aiScore}</h2>
  ...
  return string === null ? (
    <>
      <div id="page-layout" class="container-fluid">
        ...
        <button onClick={startGame} className = "return">Start the WAR!!!!!</button>
        <button onClick={fight} className = "return">Fight Wave!</button>
